...
def extract_blocks_info(blocks):
    ...
    # Show sample of first few non-empty blocks
    # sample_blocks = [b for b in blocks_array if b['ltable_count'] > 0 or b['rtable_count'] > 0][:3]
    # for block in sample_blocks:
    #     log_print(f"Sample block {block['block_id']}: left={block['ltable_ids'][:5]} right={block['rtable_ids'][:5]}")
...
def generate_block_pairs(blocks_array, match_df):
    ...
    # PyJedAI assigns continuous IDs, so we need to find where right table starts
...
def run_blocking_method(method, left_df, right_df, match_df, dataset, blocking_attributes, partition):
    ...
    # block cleaning 
    # log_print("Applying Block Purging...")
    # bp = BlockPurging()
    # blocks = bp.process(blocks, data, tqdm_disable=True)

    # log_print(f"Blocks after purging: {len(blocks)}")

    # log_print("Applying Block Filtering...")
    # bf = BlockFiltering(ratio=0.8)
    # blocks = bf.process(blocks, data, tqdm_disable=True)
    
    # log_print(f"Blocks after filtering: {len(blocks)}")
