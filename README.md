...
- Introduce blocking strategies to reduce the number of context retrieval and LLM inference for entity matching
- Retrieves and serach relevant context from external knowledge graphs (e.g., Wikidata)
- Augment LLMs for entity matching with retrieved and refined context from external knolwedge base
...
### 1. Environment Setup
The project includes a `rag4em.yml` file that contains all necessary dependencies, create coda environment from YAML file.
...
3. **Outputs** (`output/`): Construct prompt with differnt retreival context, output of LLM inference, the final restuls with evaluation metrics
...
### Dataset
The abt, amgo, beer, dbac, dbgo, foza, itam, waam datasets and wdc dataset originated from the following works, 
```
