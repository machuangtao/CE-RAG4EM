"""
Collect every unique QID/PID referenced in retrieval_outputs/*/*_test_falcon_retrieval.json
and fetch each one's English label + description from the official Wikidata API,
storing everything in one unified JSON file.

Usage:
    python falcon_entity_metadata.py
"""
...
def main():
    parser = argparse.ArgumentParser(description="Fetch Wikidata labels/descriptions for Falcon-retrieved QIDs/PIDs.")
    parser.add_argument("--input-glob", default="retrieval_outputs/*/*_test_falcon_retrieval.json")
    parser.add_argument("--output", default="retrieval_outputs/falcon_entity_metadata.json")
    ...
    logger = logging.getLogger("falcon_metadata")
    logger.setLevel(logging.INFO)
    fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s")
    file_handler = logging.FileHandler("logs/falcon_entity_metadata.log")
    ...
    pending_ids = [i for i in all_ids if i not in output["entities"]]
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="falcon_metadata"):
